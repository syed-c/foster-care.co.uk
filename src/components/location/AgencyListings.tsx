...
export const AgencyListings = ({ 
  ...
  return (
    <section className="py-16 md:py-20 bg-secondary" id="agencies">
        ...
        >
          <div>
            <div className="flex items-center gap-3 mb-3">
              ...
              {showFeaturedLabel && (
                <Badge className="bg-amber-500/20 text-amber-300 border-amber-500/30 rounded-full">
        ...
        >
          {agencies.map((agency) => (
            <motion.div 
              ...
              className="group bg-white/5 hover:bg-white/10 border border-white/10 hover:border-primary/40 rounded-2xl p-4 md:p-5 transition-all duration-300"
            >
              <div className="flex items-center gap-4 md:gap-6">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1.5 flex-wrap">
                    ...
                    {agency.is_verified && (
                      <Badge className="bg-verified/20 text-verified border-verified/30 text-xs rounded-full">
                        <CheckCircle className="w-3 h-3 mr-1" />
                        Verified
                      </Badge>
                    )}
                    {agency.is_featured && (
                      <Badge className="bg-amber-500/20 text-amber-300 border-amber-500/30 text-xs rounded-full">
                  ...
                  <div className="flex items-center gap-4 text-sm text-white/50">
                    ...
                    {agency.description && (
                      <span className="hidden lg:block truncate max-w-lg text-white/40">
                  ...
                  {agency.rating && agency.rating > 0 ? (
                    <div className="text-center">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        ...
                        <span className="font-bold text-xl text-white">{agency.rating.toFixed(1)}</span>
                      </div>
                ...
                <div className="flex items-center gap-2 md:gap-3">
                  {agency.phone && (
                    <Button 
                      size="sm" 
                      variant="outline" 
                      className="border-white/20 text-white hover:bg-white/10 hidden md:flex rounded-xl"
                      asChild
                    >
