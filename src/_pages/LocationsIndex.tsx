  ...
  return (
    <motion.div variants={itemVariants}>
      <div className="h-full group bg-slate-800/50 hover:bg-slate-800 border border-slate-700/50 hover:border-slate-600 rounded-2xl overflow-hidden transition-all duration-300">
        <Link href={`/locations/${country.slug}`} className="block p-6">
              ...
              >
                <div className="grid grid-cols-2 gap-2">
                  {regions.slice(0, 8).map((region) => (
                    <Link
                      key={region.id}
                      href={`/locations/${country.slug}/${region.slug}`}
...
export default function LocationsIndex() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (searchResults.length === 1) {
      router.push(`/locations/${searchResults[0].slug}`);
      setShowResults(false);
    } else if (searchQuery.trim()) {
      router.push(`/agencies?search=${encodeURIComponent(searchQuery)}`);
    }
  };
